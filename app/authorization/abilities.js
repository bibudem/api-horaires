...
export function getRoleFor(userOrGroup, strictMatch = false) {
  ...
  if (userOrGroup) {
    const normalizedUserLogin = normalizeLogin(userOrGroup.login)
    for (const item of users) {
      ...
      if ('login' in item) {
        const normalizedConfigLogin = normalizeLogin(item.login)
        if (normalizedUserLogin && normalizedUserLogin === normalizedConfigLogin) {
